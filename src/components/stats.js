    ...
    render() {
        ...
        const generalStats = (this.props.getLoggedIn()) ? (
                <div id="generalStats" className="container row">
                    <div id="siteGoal" className="offset-3 col-3 row" title="Your progress to reaching your site's goal for each agent during AEP">
                        <div id="siteGoalBreakdown" className="col-8 row">
                            <span className="col-6">MANE:</span>
                            <span className="col-6">{ this.enrollments(conversions.aepToDate, 'MA', 'P') }</span>
                            <br />
                            <span className="col-6">Goal:</span>
                            <span className="col-6">{ this.state.siteGoal[sessionStorage.getItem('client')][sessionStorage.getItem('site')] }</span>
                        </div>
                        <span id="siteGoalPercent" className="col-4">
                            { Number(this.enrollments(conversions.aepToDate, 'MA', 'P') /
                                this.state.siteGoal[sessionStorage.getItem('client')][sessionStorage.getItem('site')] * 100).toFixed(2)}%
                        </span>
                    </div>
                    <div id="gpEntries" className="offset-1 col-5" title="Entries into Grand Prize raffle - 1 for every 5 MANE enrollments">
                        Grand Prize Entries: { Math.floor(
                            ( this.enrollments(conversions.aepToDate, 'MA', 'P')
                                + this.enrollments(conversions.aepToDate, 'PDP', 'P') )
